...
// ==============================================
// PHYSICAL PROGRESS VALIDATION RULES
// ==============================================

// Validation rules for updating physical progress
export const validateProgressUpdate = [
  ...
  body("progress").custom((value, { req }) => {
    ...
    if (
      progress === 100 &&
      (!req.firebaseFiles || req.firebaseFiles.length === 0)
    ) {
      throw new Error(
...
// ==============================================
// FINANCIAL PROGRESS VALIDATION RULES
// ==============================================
...
// ==============================================
// COMBINED PROGRESS VALIDATION RULES
// ==============================================
...
export const validateProgressStatisticsQuery = [
  ...
  query("typeOfWork")
    .optional()
    .isLength({ min: 2, max: 100 })
    .withMessage("Type of work must be between 2 and 100 characters")
    .trim(),

  query("natureOfWork")
    .optional()
    .isLength({ min: 2, max: 100 })
    .withMessage("Nature of work must be between 2 and 100 characters")
    .trim(),
  ...
  if (!errors.isEmpty()) {
    ...
    return res.status(400).json({
      ...
      details: {
        ...
        validationType: "project_progress",
      },
    });
  }
...
// Middleware to handle financial validation errors
...
// Business logic validation middleware for progress
...
export const validateCompleteProgressUpdate = [
...
// Comprehensive validation middleware for financial progress
...
// Validation for project deadlines
export const validateProjectDeadline = async (req, res, next) => {
  try {
    const { progress } = req.body;
    const projectId = req.params.id;

    if (progress && parseFloat(progress) === 100) {
      // When marking project as 100% complete, check if it's within deadline
      const Project = require("../../models/project.model.js").default;
      ...
      if (project) {
        ...
        if (effectiveDeadline && currentDate > effectiveDeadline) {
          req.progressWarnings = req.progressWarnings || [];
          req.progressWarnings.push({
            type: "COMPLETED_AFTER_DEADLINE",
            message: `Project completed after deadline (${effectiveDeadline.toDateString()})`,
            severity: "warning",
    ...
    console.error("Error in project deadline validation:", error);
    next(error);
  }
};
