...
/**
 * Middleware for uploading avatar to AWS S3
 * Uploads single image file and adds the URL to req.body.avatar
 */
export const uploadAvatarToS3 = async (req, res, next) => {
  ...
  multerMiddleware(req, res, async (multerError) => {
    ...
    try {
      ...
      console.log(`Uploading avatar to S3 folder: ${avatarFolder}`);
      ...
      if (uploadedFiles.length > 0) {
        // Process the uploaded file to add download URL
      ...
      if (req.uploadedAvatarFile) {
        deleteMultipleFilesFromS3([req.uploadedAvatarFile.filePath]).catch(
          (deleteError) => {
            console.error(
              "Error cleaning up failed avatar upload:",
              deleteError
            );
          }
        );
      ...
      return res.status(500).json({
        ...
        details: {
          error: error.message,
          errorCode: "S3_AVATAR_UPLOAD_ERROR",
        },
      });
    }
  });
