...
/**
 * Middleware for uploading files to S3 Storage
 *
 * Files are uploaded to: {baseFolder}/{updateType}/{projectId}/{YYYY-MM-DD}/
 *
 * Examples:
 * - uploads/progress-updates/project-123/2025-10-07/
 * - uploads/financial-updates/project-456/2025-10-07/
 *
 * @param {string} fieldName - Form field name for files (default: "supportingFiles")
 * @param {number} maxCount - Maximum number of files (default: 10)
 * @param {string} folder - S3 base folder (default: "uploads")
 */
export const uploadFilesToFirebase = (
  ...
  return async (req, res, next) => {
    ...
    multerMiddleware(req, res, async (multerError) => {
      ...
      try {
        // Parse FormData objects after multer has processed the request
        ...
        console.log(
          `Uploading ${req.files.length} files to S3 folder: ${contextFolder}`
        );
        ...
        req.firebaseFiles = processS3Files(uploadedFiles);
        req.uploadedFiles = req.firebaseFiles; // For backward compatibility

        console.log(
          `Successfully uploaded ${uploadedFiles.length} files to S3`
        );
        next();
      } catch (error) {
 ...
 *
 * Folder structure: {baseFolder}/{updateType}/{projectId}/{YYYY-MM-DD}
 *
 * Examples:
 * - uploads/progress-updates/project-123/2025-10-07
 * - uploads/financial-updates/project-456/2025-10-07
 * - uploads/progress-updates/general/2025-10-07
 *
 * @param {Object} req - Express request object
 * @param {string} baseFolder - Base folder name (default: "uploads")
 * @returns {string} - Complete folder path in S3
 */
...
/**
 * Enhanced error handling middleware for S3 uploads
 */
...
export const validateFirebaseFileUpload = (req, res, next) => {
  try {
    ...
    // FIXED: Map S3 property names to expected property names
    if (uploadedFiles.length > 0) {
      req.firebaseFiles = uploadedFiles.map((file) => ({
        ...file,
      ...
      console.log(
        `Successfully processed ${req.firebaseFiles.length} files for project ${req.params.id}`
      );
      req.firebaseFiles.forEach((file, index) => {
        console.log(
    ...
    console.error("Error in S3 file upload validation:", error);
    return res.status(500).json({
      success: false,
...
/**
 * Cleanup middleware to delete uploaded files from S3 in case of subsequent errors
 */
  ...
  res.send = function (data) {
    if (
      ...
      deleteMultipleFilesFromS3(filePaths).catch((error) => {
        console.error(
          "Error cleaning up S3 files after error response:",
          error
        );
      });
    }
...
export const processUploadedFiles = (files) => processS3Files(files);
export const deleteUploadedFiles = async (files) => {
  const filePaths = files.map((f) => f.filePath || f.storageRef);
  return deleteMultipleFilesFromS3(filePaths);
};
